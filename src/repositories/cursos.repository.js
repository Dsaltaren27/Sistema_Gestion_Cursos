...
async function findAll() {
  const result = await pool.query(
    'SELECT c.id, c.nombre AS curso_nombre, c.descripcion, c.profesor_id, u.nombre AS profesor_nombre FROM cursos c JOIN usuarios u ON u.id = c.profesor_id'
  );
  return result.rows;
}
...
async function findById(id) {
  const result = await pool.query(
    'SELECT c.id, c.nombre AS curso_nombre, c.descripcion, c.profesor_id, u.nombre AS profesor_nombre  FROM cursos c JOIN usuarios u ON u.id = c.profesor_id WHERE c.id = $1',
    [id]
  );
